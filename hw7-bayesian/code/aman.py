...
def likelihoodFunc(W, x, y_train, likelihood_var):
    ...
    for i in range(len(x)):
        mu = np.dot(W.T, x[i, :].T)
        normal = 1.0 / np.sqrt(2 * likelihood_var * np.pi)
        normal = normal * np.exp(-1.0 * (mu - y_train[i]) ** 2 / (2 * likelihood_var))
        likelihood *= normal
...
def getPosteriorParams(x, y_train, prior, likelihood_var = 0.2**2):
    ...
    '''
    postMean = (np.dot(x.T, x) + likelihood_var * likelihood_var * prior['var'].getI()).getI()
    postMean = postMean.dot(x.T).dot(y_train)
    postVar = ((1.0 * np.dot(x.T, x) / (likelihood_var * likelihood_var)) + prior['var'].getI()).getI()
...
def getPredictiveParams(x_new, postMean, postVar, likelihood_var = 0.2**2):
    ...
    x = x_new.reshape(-1,1)
    predMean = np.dot(postMean.T, x)
    predVar = x.T.dot(postVar).dot(x) + likelihood_var * likelihood_var
